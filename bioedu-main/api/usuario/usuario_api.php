...
function getAll($conn) {
    try {
        $stmt = $conn->query('SELECT id_usuario, nome, email, data_nascimento, usuario FROM usuarios ORDER BY nome');
...
function getById($conn, $id) {
    try {
        $stmt = $conn->prepare("SELECT id_usuario, nome, email, data_nascimento, usuario FROM usuarios WHERE id_usuario = ?");
        $stmt->execute([$id]);
...
function login($conn) {
    ...
    try {
        $stmt = $conn->prepare("SELECT id_usuario, usuario, senha FROM usuarios WHERE usuario = ?");
...
function put($conn, $id) {
    ...
    try {
        ...
        $stmt = $conn->prepare("SELECT nome, email, data_nascimento, usuario, senha FROM usuarios WHERE id_usuario = ?");
        $stmt->execute([$id]);
